...
void Level2::Initialize() {
	...
	state.map = new Map(LEVEL2_WIDTH, LEVEL2_HEIGHT, level2_data, mapTextureID, 1.0f, 9, 9);
	...
	state.enemies[1].position = glm::vec3(11, -4, 0);
	state.enemies[1].aiType = WAITANDGO;
	state.enemies[1].aiState = IDLE;
	state.enemies[2].position = glm::vec3(7, -5, 0);
...
void Level2::Render(ShaderProgram* program) {
	...
	if (state.gameEnd) {
		...
		if (state.player->position.x < 5) {
			...
			showLife = glm::vec3(state.player->position.x - 0.6, -3.75, 0);
		}
		if (state.win) {
