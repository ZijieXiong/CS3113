...
void ProcessInput() {
	if (currentScene != sceneList[0]) {
		...
		currentScene->state.player->isUp = false;
		currentScene->state.player->isDown = false;
		currentScene->state.player->isRight = false;
		currentScene->state.player->isLeft = false;
	}
	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_KEYDOWN:
			...
			case SDLK_RETURN:
				...
				break;
			/*case SDLK_w:
				currentScene->state.player->isUp = true;
				break;
			case SDLK_s:
				currentScene->state.player->isDown = true;
				break;
			case SDLK_a:
				currentScene->state.player->isLeft = true;
				break;
			case SDLK_d:
				currentScene->state.player->isRight = true;
				break;*/
			}
			break;
...
void Update() {
	...
	if (currentScene != sceneList[0]) {
		...
		if (currentScene->state.player->position.x < 5) {
			viewMatrix = glm::translate(viewMatrix, glm::vec3(-5+currentScene->state.player->position.x, 0, 0));

		}
		else if (currentScene->state.player->position.x > 8) {
			viewMatrix = glm::translate(viewMatrix, glm::vec3(-8+currentScene->state.player->position.x, 0, 0));
		}
		if (currentScene->state.player->position.y < -4) {
			viewMatrix = glm::translate(viewMatrix, glm::vec3(0, 4 + currentScene->state.player->position.y, 0));
		}
		else if (currentScene->state.player->position.y > -3.5) {
			viewMatrix = glm::translate(viewMatrix, glm::vec3(0, 3.5 + currentScene->state.player->position.y, 0));
		}
	}
...
void Render() {
	...
	glm::vec3 showLife = glm::vec3(currentScene->state.player->position.x - 4, currentScene->state.player->position.y + 3, 0);
	if (currentScene->state.player->position.x < 5) {
		showLife.x = 1;
